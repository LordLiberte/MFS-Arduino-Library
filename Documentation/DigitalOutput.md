...
> Una salida digital con modos de parpadeo no bloqueante, forzado y un watchdog que enclava. Y, al final del archivo, `AnalogOutput` con rampa, que es lo que evita que el robot se reinicie solo.
...
2. **Protegerse de quedarse pegada.** Una electroválvula excitada dos horas se
   quema. Una resistencia, peor.
...
da `HIGH`. Con `activeLow = true` (un módulo de relé de los chinos), `true` da
`LOW`.

Y `begin()` hace `applyPhysical(false)` **antes de nada**: sin esa línea, un
módulo de relé activo a bajo arrancaría pegado durante todo el tiempo que tarda
el primer `writeOutputs()`.
...
|---|---|---|
| constructor | `DigitalOutput(uint8_t pin, bool activeLow = false)` | |
...
| `mode()` | `OutputMode` | |
| `setMaxOnTime(ms)` | | Watchdog. `0` = sin límite |
| `hasTimedOut()` / `clearTimeout()` | | Estado y rearme del watchdog |
...
| Miembro | Bytes |
|---|---:|
| `_pin`, `_activeLow`, `_mode`, `_physical`, `_simValue`, `_timedOut` | 6 |
| `_maxOnTimeMs`, `_onSince` | 8 |
| `_blink` | lo que ocupe `Blink` (unos 6) |
| `IDevice` | 5 |

**Unos 25 bytes.** `AnalogOutput` es más ligero: sin `Blink` ni watchdog, ronda
los 16.
