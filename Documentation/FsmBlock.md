...
`abort()` corta en seco, que es lo que hace una seta de emergencia.
...
Esto evita el vicio clásico de darle al botón de rearme cincuenta veces con la
seta todavía hundida, y de paso obliga a que el rearme signifique algo.
...
replegar. `abort()` corta en seco. Para una seta, `abort()`; para el botón de
paro del panel, `stop()`.
