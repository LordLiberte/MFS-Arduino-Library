...
estructura de bits: es *implementation-defined*. En la práctica GCC —que es el
compilador de AVR, ESP32, RP2040 y SAMD por igual— asigna empezando por el bit
menos significativo, así que `enable` cae en el bit 0 y `start` en el 1.

Como esta librería solo se compila con GCC, el reparto es estable y la unión es
segura. Pero si vas a mandar estas palabras **a un dispositivo de otro
fabricante compilado con otro compilador**, no te fíes: usa las máscaras
explícitas `CFGW_BIT_*` / `STW_BIT_*`, que sí son portables al cien por cien.
...
El motivo es que **el fallo tiene que llevar al estado seguro**. Si el cable se
corta, si el bus se cae, si una trama se pierde, lo que llega es una palabra a
cero. Con la lógica normal, cero significaría "no me han pedido parar" y la
máquina seguiría corriendo con el mando desconectado. Con la lógica invertida,
cero significa parada y la máquina se para sola.

Es el mismo criterio con el que se cablea un circuito de seta de emergencia:
contacto normalmente cerrado, de modo que un cable roto abre el circuito y para
la máquina. La seguridad no se diseña para que funcione, se diseña para que
falle bien.
