...
Un `analogRead()` crudo devuelve un número entre 0 y 1023 que **no significa
nada**, tiembla, y no distingue "vale cero" de "el cable está cortado". Este
...
de ciclos de reloj y se emula por software. Y no aporta nada: un ADC de 10 bits
tiene 1024 valores distintos, así que la precisión real no llega ni a tres
cifras significativas. Guardar bares en centésimas (`1000` = 10,00 bar) da toda
la resolución que el sensor puede ofrecer, en aritmética entera.
...
identidad (0-1023 → 0-1023), así que `scaled()` devuelve cuentas y parece que
funciona. Luego alguien cambia el sensor y nada cuadra.
