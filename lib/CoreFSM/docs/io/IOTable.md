...
**Lo genera:** `tools/corefsm_gen.py` desde CSV, JSON o Wokwi;
`wokwi2corefsm.py` mantiene compatibilidad.
...
El `? : 1` mantiene válida la capacidad de dispositivos cuando no hay señales.
Los backends se cuentan por separado; el propio manager protege también su
array cuando `CFSM_BACKEND_COUNT` es cero.
...
struct CfsmHardware {
  ...
  DeviceManager<CFSM_IO_COUNT ? CFSM_IO_COUNT : 1, 0> devices; // 3 dispositivos
...
Escribe `CFSM_DEFINE_HARDWARE;` con punto y coma. La macro ya lo contiene por
compatibilidad, pero el separador visible evita que algunos conversores `.ino`
la confundan con parte del tipo de retorno de `setup()` cuando va justo antes.
...
| `CFSM_TABLE_BACKEND(ROW)` | `ROW(tipo, Nombre, argumentos...)` |
| `CFSM_TABLE_DI_BACKEND(ROW)` | `ROW(backend, canal, Nombre, pullup, antirreboteMs)` |
| `CFSM_TABLE_DO_BACKEND(ROW)` | `ROW(backend, canal, Nombre, activoBajo, valorSeguro)` |
| `CFSM_TABLE_DO_SAFE(ROW)` | `ROW(pin, Nombre, activoBajo, valorSeguro)` |
...
| `HW.setSafetyInterlock(bool)` / `HW.isSafetyInterlocked()` | Corte global de software |
| `HW.allBackendsHealthy()` | Salud conjunta de backends registrados |
| `HW.releaseAllForces()` / `HW.hasAnyForce()` | Forzados |
| `HW.printIoTable(Print&)` | Volcado de la imagen de proceso |
| `CFSM_DI_COUNT`, `CFSM_DO_COUNT`, `CFSM_AI_COUNT`, `CFSM_IO_COUNT`, `CFSM_BACKEND_COUNT` | Cuentas, en compilación |
...
La fila `Seta_Emergencia` solo sirve para informar a la lógica. Ni un filtro de
cero ni `setSafetyInterlock()` convierten el MCU en un dispositivo de seguridad:
la retirada de energía peligrosa debe realizarse por una cadena independiente.
...
**Usar una fila DI como única parada de emergencia.** Ver §4.1 y
[SAFETY.md](../../../../SAFETY.md).

**Meter a mano una fila con un pin inexistente o repetido.** `IOTable.h` no puede
validarlo. El generador neutral sí rechaza destinos duplicados y opciones
incompatibles, pero la existencia física del pin sigue dependiendo de la placa.
...
| `DigitalOutput` | ~14 B |
| `AnalogSensor` | ~28 B |
| `DeviceManager<N,B>` | punteros para N dispositivos y B backends, más flags |

Los tamaños históricos dejaron de ser fiables al añadir `DigitalPin`, estado
seguro y backends. Las capacidades continúan siendo exactas en compilación,
pero mide los bytes con el toolchain de la placa objetivo.

## 8. Relación con el resto

```
   hardware.csv / hardware.json / diagram.json
        │  tools/corefsm_gen.py
        ▼
   HardwareConfig.h        ← tu tabla, generada o a mano
        │  define CFSM_TABLE_DI / DO / AI y variantes de backend
        │  y luego incluye
        ▼
   ...
   struct CfsmHardware  ──▶  HW
        ...
        └── DeviceManager<CFSM_IO_COUNT, CFSM_BACKEND_COUNT> (la imagen)
