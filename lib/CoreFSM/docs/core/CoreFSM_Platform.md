...
### 2.2 Resolución ADC por defecto

`CFSM_ADC_MAX` vale 4095 en ESP32 y 1023 en el resto de plataformas. Es el
extremo que usa `AnalogSensor` para su escala y rango válido iniciales. Si el
sketch llama a `analogReadResolution()` o el core usa otra resolución, define
`CFSM_ADC_MAX` antes de incluir la librería o mediante `-D`.

### 2.3 La capa de memoria no volátil
...
### 2.4 El reloj, y la resta que sobrevive al desbordamiento
...
### 2.6 Cadenas en memoria de programa
...
| `CFSM_ADC_MAX` | entero | Cuenta máxima ADC predeterminada o definida por el proyecto |
...
**Comparar sumando en vez de restando.** Ya explicado en 2.4. Si ves un
