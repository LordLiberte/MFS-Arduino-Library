...
| `onEmergencyStop()` | `virtual void onEmergencyStop()` | Difusión del interbloqueo lógico |
...
### 4.2 Un bloque que recibe el interbloqueo sin ser un `FsmBlock`
...
Sin ese `onEmergencyStop()`, la orden `resistencia` conservaría su último valor:
con el interbloqueo activo `updateAll()` **no llama a `update()`**. Este hook
solo cambia una variable de software; hace falta conectarla a la salida y
ejecutar la PAA. Para objetos de `CfsmHardware`, enlaza además
`HW.setSafetyInterlock(manager.isEmergencyStop())`. Nada de ello sustituye el
corte de energía cableado descrito en [`SAFETY.md`](../../../../SAFETY.md).

### 4.3 Poner una estación fuera de servicio desde la consola

```cpp
bool sacarEstacion1 = false;

consola.setExtraHandler([](char c) {
  if (c == '1') estacion1.disable();
  if (c == '2') { estacion1.stop(); sacarEstacion1 = true; }
  if (c == '0') estacion1.enable();
});

// Después de manager.updateAll():
if (sacarEstacion1 && estacion1.getState() == STATE_STOPPED) {
  estacion1.disable();
  sacarEstacion1 = false;
}
```

La orden `'1'` congela la estación tal como esté y puede conservar mandos
activos. La orden `'2'` solicita primero la parada y solo la excluye del scan
cuando ha llegado a `STOPPED`. Aun así, valida los actuadores y su PAA: poner un
bloque fuera de servicio no es una función de seguridad.
