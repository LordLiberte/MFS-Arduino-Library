...
export function createGrid(config) {
  ...
  let editing = null;   // {row, col, input, previous}
  ...
  function edit(rowIndex, colIndex) {
    ...
    field.addEventListener("keydown", (event) => {
      ...
      else if (event.key === "Enter") { event.preventDefault(); commit(); move(rowIndex + 1, colIndex); }
  ...
  function move(rowIndex, colIndex, direction = 1) {
    ...
    if (rowIndex >= rows.length) { append(); return; }
    const columns = columnsOf();
    let ci = colIndex;
    ...
    if (direction > 0) return move(rowIndex + 1, 0, 1);
  ...
  function cancel() {
    ...
    /* Escape sobre la última fila recién añadida y todavía sin nombre la
     * descarta: si no, quedaría una fila vacía que impide guardar y que el
     * usuario tendría que borrar a mano después de haber dicho «déjalo». */
    const key = columnsOf()[0].key;
    if (target && row === rows.length - 1 && !String(target[key] || "").trim()) {
      rows.splice(row, 1);
  ...
  function append() {
    ...
    const rows = config.rows();
    rows.push(config.newRow(rows.length));
    if (config.onChange) config.onChange(null, null, null);
    render();
...
export function gridToolbar({ title, actions = [], onSearch, count }) {
  ...
  if (count !== undefined) bar.appendChild(h("span", { class: "muted" }, count + (count === 1 ? " fila" : " filas")));
