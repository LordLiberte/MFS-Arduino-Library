...
export function createEditor(options = {}) {
  ...
  function highlight(text) {
    const symbolNames = state.symbols ? state.symbols.objects : new Map();
    const steps = new Set((state.symbols && state.symbols.model ? state.symbols.model.states || [] : [])
  ...
  function render() {
    ...
    for (let i = 0; i < lines.length; i++) {
      const cls = "ln" + (bad.has(i + 1) ? " bad" : "") + (i === cur ? " cur" : "");
  ...
  input.addEventListener("click", () => { closeComp(); closeSig(); render(); });
  ...
  return {
    ...
    setValue(text) { input.value = text; render(); },
    setReadOnly(flag) {
      state.readOnly = flag; input.readOnly = flag;
      root.classList.toggle("readonly", flag);
    },
    setSymbols(symbols) { state.symbols = symbols; render(); },
