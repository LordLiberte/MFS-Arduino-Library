  ...
  return h("div", { class: "portal" },
    h("div", { class: "portal-inner" },
      ...
      h("div", { class: "tasks" },
        ...
        task("link", "Importar los pines", "Trae la configuración desde el diagram.json de Wokwi, un CSV exportado de cualquier programa de esquemáticos o un hardware.json.", async () => {
          const last = state.projectId || (state.projects[0] && state.projects[0].id);
          if (!last) { toast("Crea primero un proyecto.", "warn"); return; }
          if (!state.projectId) await actions.openProject(last, "hardware");
          actions.openImporter();
        }),
