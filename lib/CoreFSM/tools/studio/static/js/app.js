...
import { state, emit, emitNow, subscribe } from "./store.js";
...
import { renderMonitor, refreshPorts, stopPolling, startPolling } from "./views/monitor.js";
import { renderInspector } from "./inspector.js";
import { openImporter } from "./views/importer.js";
...
const actions = {
  openProject, createProject, go, save, generate, build, upload,
  newProject: () => newProject(actions), openImporter,
};
...
function renderRibbon() {
  ...
  bar.appendChild(h("div", { class: "group" },
    button("Importar", { icon: "upload", title: "Traer los pines desde Wokwi, un CSV o un hardware.json",
...
function treeState() {
  const m = state.model;
  return [state.projectId, state.view, state.activeTab, state.dbIndex,
    m.hardware.length, m.devices.length, m.states.length, m.udts.length,
    m.dataBlocks.map((b) => b.name + ":" + (b.variables || []).length).join(","),
    state.files.map((f) => f.path).join(","),
    Object.keys(state.generated).join(",")].join("|");
}

function renderTree() {
  const signature = treeState();
  if (signature === treeSignature && el("tree-body")) return;
...
function renderTabs() {
  ...
  for (const tab of state.tabs) {
    ...
    strip.appendChild(h("button", {
      ...
      onclick: () => { state.activeTab = tab.key; state.view = tab.kind === "file" ? "code" : tab.kind; emitNow({ view: true }); },
    }, icon(tab.icon || "file"), h("span", {}, tab.label),
       h("span", { class: "x", onclick: (e) => { e.stopPropagation(); S.closeTab(tab.key); emitNow({ view: true }); } }, "×")));
  }
}
...
function renderBottom() {
  ...
  if (!state.bottomOpen) { clear(el("bottom-body")); return; }
  const consoleSignature = state.bottomTab + "|" + state.console.length + "|" +
    (state.validation.errors || []).length + "|" + (state.validation.warnings || []).length;
  if (consoleSignature === bottomSignature) return;
  bottomSignature = consoleSignature;
  ...
  if (state.bottomTab === "diag") {
    ...
    /* Una compilación de PlatformIO son miles de líneas. Repintarlas todas en
     * cada aviso bloquea la interfaz, así que se muestran las últimas y solo
     * cuando la consola ha crecido de verdad. */
    const visible = state.console.slice(-400);
    body.appendChild(h("div", { class: "console" },
      state.console.length > visible.length
        ? h("div", { class: "dim" }, "… " + (state.console.length - visible.length) + " líneas anteriores")
        : null,
      ...visible.map((line) => h("div", { class: line.kind },
...
function go(view) {
  if (view === "monitor") {
    if (state.monitor.available === null) refreshPorts();
    if (state.monitor.connected) startPolling();
  } else if (state.monitor.timer) {
    // Sondear el puerto mientras se está en otra vista repinta cuatro veces por
    // segundo sin que nadie lo vea; el hilo del servidor sigue acumulando.
    stopPolling();
  }
  state.view = view;
